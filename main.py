...
Ranchero.NeighLib.print_col_where(tba6, 'run_index', 'ERR176605') # list
Ranchero.NeighLib.print_col_where(tba6, 'run_index', 'SRR11565459') # range
Ranchero.NeighLib.print_col_where(tba6, 'run_index', 'SRR25514982') # ideal
Ranchero.NeighLib.print_col_where(tba6, 'run_index', 'ERR495032') # empty
...
print(f"Rancheroized and wrote to disk in {time.time() - start}s seconds")
...
print(PRJNA834606.columns)

merged = Ranchero.merge_polars_dataframes(tba6, coscolla, merge_upon="run_index", left_name="tba6", right_name="Coscolla", put_right_name_in_this_column="source")
...
merged_by_sample = Ranchero.NeighLib.rancheroize_polars(merged_by_sample)


print(merged_by_sample.select(['date_collected', 'sample_index']))
Ranchero.NeighLib.print_col_where(tba6, 'sample_index', 'SAMN02586062') # multiple run accessions going into same date
Ranchero.NeighLib.print_col_where(tba6, 'sample_index', 'SAMN33804027') # 1905???
...
start, merged_by_sample = time.time(), Ranchero.hella_flat(merged_by_sample, verbose=True)
