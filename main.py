...
def run_merges(tba6):
	...
	nimmo_L2 = Ranchero.from_tsv("./inputs/publications/run_indexed/Nimmo_2024/L2.tsv", auto_rancheroize=True)
	nimmo_L2 = Ranchero.NeighLib.add_column_of_just_this_value(nimmo_L2, "lineage", "L2")
	nimmo_L4 = Ranchero.from_tsv("./inputs/publications/run_indexed/Nimmo_2024/L4.tsv", auto_rancheroize=True)
...
def sample_index_merges(merged_runs):
	...
	Andres_pheno = Ranchero.from_tsv("./inputs/publications/sample_indexed/Andres_2019/Andres_cleaned.tsv", drop_columns=['Ethambutol_MIC'], auto_rancheroize=True)
	Andres_pheno = Ranchero.NeighLib.add_column_of_just_this_value(Andres_pheno, "pheno_source", "Andres_2019_PMC6355586")
	start, merged = time.time(), Ranchero.merge_dataframes(merged, Andres_pheno, merge_upon="sample_index", right_name="finci_2022", indicator="collection")
	...
	standford_4 = Ranchero.standardize_countries(Ranchero.cleanup_dates(Ranchero.from_tsv("./inputs/max_standford_slashdates.tsv", auto_rancheroize=True, auto_parse_dates=True))) # ditto
...
if sample_merges:
	...
	start, merged_samps = time.time(), Ranchero.from_tsv("ranchero_partial_rc5.tsv")
	print(f"Imported run-indexed tba6 file, with some merges, in {time.time() - start:.4f} seconds")
...
merged = merged_samps
merged = merged.drop(['lat', 'lon', 'date_collected_year', 'date_collected_month', 'reason', 'host_info', 'geoloc_name'])
Ranchero.to_tsv(merged, "./ranchero_rc6.tsv")
...
Ranchero.NeighLib.print_value_counts(merged, ['date_collected'])
Ranchero.NeighLib.print_value_counts(merged, ['mycobact_type', 'organism', 'lineage', 'strain'])
