...
def run_merges(tba6):
	...
	#shuaib_XRS = Ranchero.from_tsv("./inputs/publications/sample_ided/Shuaib_2022 (PMC9222951)/shuaib_2022_XRS.tsv", auto_rancheroize=False, glob=False, check_index=False)
	...
	shuaib_SRR = Ranchero.from_tsv("./inputs/publications/sample_ided/Shuaib_2022 (PMC9222951)/shuaib_2022_SRR.tsv", auto_rancheroize=False, glob=False, check_index=False)
	merged = Ranchero.standardize_countries(Ranchero.merge_dataframes(merged, shuaib_SRR, merge_upon="run_id", right_name="Shuaib_2022", indicator="collection"))

	# Bos
	#bos = Ranchero.from_tsv("./inputs/publications/Bos_2015/ancient.tsv")
	#merged = Ranchero.merge_dataframes(merged, bos, merge_upon="run_id", left_name="tba6", right_name="Bos (ancient)")

	# Brites
	print(f"{_b_}Processing Brites{_bb_}")
	brites = Ranchero.from_tsv("./inputs/publications/run_ided/Brites_2018/brites_cleaned.tsv", 
		auto_rancheroize=True, explode_upon=";", 
		drop_columns=['country']) # several incorrect countries
	merged = Ranchero.merge_dataframes(merged, brites, merge_upon="run_id", left_name="tba6", right_name="Brites_2018",
		drop_exclusive_right=True) # we have to do this or else run-to-sample breaks
	check_stuff(merged)

	# Cancino-Muñoz
	print(f"{_b_}Processing Cancino-Muñoz{_bb_}")
	cancinomunoz = Ranchero.from_tsv("./inputs/publications/run_ided/Cancino-Muñoz_2022/cancino-munoz_2022.tsv")
	assert 'geoloc_info' not in cancinomunoz.columns
	assert 'geoloc_info' not in merged.columns
	merged = Ranchero.merge_dataframes(merged, cancinomunoz, merge_upon="run_id", left_name="tba6", right_name="Cancino-Muñoz_2022",
		drop_exclusive_right=True)
	check_stuff(merged)

	# Coll
	print(f"{_b_}Processing Coll{_bb_}")
	coll = Ranchero.from_tsv("./inputs/publications/run_ided/Coll_2018/coll_processed.tsv", auto_standardize=True)
	coll = Ranchero.add_column_with_this_value(coll, "pheno_source", "Coll_2018")
	merged = Ranchero.merge_dataframes(merged, coll, merge_upon="run_id", left_name="tba6", right_name="Coll_2018", drop_exclusive_right=True)

	# Coscolla
	print(f"{_b_}Processing Coscolla{_bb_}")
	coscolla = Ranchero.from_tsv("./inputs/publications/run_ided/Coscolla_2021/coscolla_sans_weird.tsv", explode_upon=";", auto_standardize=True)
	merged = Ranchero.merge_dataframes(merged, coscolla, merge_upon="run_id", left_name="tba6", right_name="Coscolla_2021", drop_exclusive_right=True)

	# CRyPTIC Reuse Table (NOT WALKER 2022)
	print(f"{_b_}Processing CRyPTIC reuse table{_bb_}")
	CRyPTIC = Ranchero.from_tsv("./inputs/publications/run_ided/CRyPTIC reuse table/CRyPTIC_reuse_table_20240917.csv", delimiter=",", explode_upon=".",
	...
	merged = Ranchero.merge_dataframes(merged, CRyPTIC, merge_upon="run_id", left_name="tba6", right_name="CRyPTIC_reuse_table", drop_exclusive_right=True)
	check_stuff(merged)

	# Merker (two of them...)
	#print(f"{_b_}Processing the run-indexed part of Merker 2022{_bb_}")
	#merker = Ranchero.from_tsv("./inputs/publications/run_ided/Merker_2022 (run)/Merker_clean_run_indeces.tsv", auto_standardize=True, 
	...
	#merged = Ranchero.merge_dataframes(merged, merker, merge_upon="run_id", left_name="tba6", right_name="Merker_2022", drop_exclusive_right=True)
	# DONT FORGET THE OTHER MERKER PAPER, AND ALSO THE SAMPLE-INDEXED PART
	#check_stuff(merged)

	# Napier
	print(f"{_b_}Processing Napier{_bb_}")
	napier = Ranchero.from_tsv("./inputs/publications/run_ided/Napier_2020/napier_samples_github_sans_weird_no_US.csv", delimiter=",", explode_upon="_", auto_standardize=True)
	merged = Ranchero.merge_dataframes(merged, napier, merge_upon="run_id", right_name="Napier_2020", drop_exclusive_right=True)
	check_stuff(merged)

	# Nimmo
	print(f"{_b_}Processing Nimmo{_bb_}")
	nimmo_L2 = Ranchero.from_tsv("./inputs/publications/run_ided/Nimmo_2024/L2.tsv", auto_standardize=False)
	...
	nimmo_L4 = Ranchero.from_tsv("./inputs/publications/run_ided/Nimmo_2024/L4.tsv", auto_standardize=False)
	...
	merged = Ranchero.merge_dataframes(merged, nimmo_L2, merge_upon="run_id", right_name="Nimmo_2024", drop_exclusive_right=True, fallback_on_left=True)
	merged = Ranchero.merge_dataframes(merged, nimmo_L4, merge_upon="run_id", right_name="Nimmo_2024", drop_exclusive_right=True, fallback_on_left=True)
	check_stuff(merged)
	
	# Stucki
	print(f"{_b_}Processing Stucki{_bb_}")
	stucki = Ranchero.from_tsv("./inputs/publications/run_ided/Stucki_2016 (run)/Stucki_2016 - cleaned - by run.tsv", auto_standardize=True)
	merged = Ranchero.merge_dataframes(merged, stucki, merge_upon="run_id", right_name="Stucki_2016", drop_exclusive_right=True)
	check_stuff(merged)
	...
	walker = Ranchero.from_tsv("./inputs/publications/run_ided/Walker_2022-CRyPTIC-run/runindexed_Walker2022_pheno_WHO2021_CRyPTIC_PMC7612554.tsv")
	walker = Ranchero.add_column_with_this_value(walker, "pheno_source", "Walker_2022") #PMC7612554
	start, merged = time.time(), Ranchero.merge_dataframes(merged, walker, merge_upon="run_id", right_name="Walker_2022", indicator="collection", fallback_on_left=True, escalate_warnings=False)
	...
	merged = Ranchero.merge_dataframes(merged, nextstrain, merge_upon="run_id", right_name="nextstrain_global_tree", drop_exclusive_right=True)
...
def sample_id_merges(merged_runs):
	...
	#merged = Ranchero.merge_dataframes(merged, atypical, merge_upon="sample_id", left_name="tba6", right_name="atypical_genotypes", drop_exclusive_right=False)

	# Andres
	print(f"{_b_}Processing Andres{_bb_}")
	Andres_pheno = Ranchero.from_tsv("./inputs/publications/sample_ided/Andres_2019/Andres_cleaned.tsv", drop_columns=['Ethambutol_MIC'], auto_standardize=True)
	Andres_pheno = Ranchero.add_column_with_this_value(Andres_pheno, "pheno_source", "Andres_2019") # PMC6355586
	start, merged = time.time(), Ranchero.merge_dataframes(merged, Andres_pheno, merge_upon="sample_id", right_name="Andres_2019", indicator="collection")
	...
	Finci_pheno = Ranchero.from_tsv("./inputs/publications/sample_ided/Finci_2022 (PRJEB48275)/PRJEB48275_Finci_plus_pheno.tsv", auto_standardize=False, # needs to be false or the merge gets hella cringe
	...
	start, merged = time.time(), Ranchero.standardize_everything(Ranchero.merge_dataframes(merged, Finci_pheno, merge_upon="sample_id", right_name="Finci_2022", indicator="collection", fallback_on_left=True), skip_sample_source=True)
	check_stuff(merged)

	# Menardo (two of them...)
	print(f"{_b_}Processing Menardo (two of them){_bb_}")
	start = time.time()
	menardo_2018 = Ranchero.from_tsv("./inputs/publications/sample_ided/Menardo_2018/menardo_2018_processed.csv", delimiter=',')
	menardo_2021 = Ranchero.from_tsv("./inputs/publications/sample_ided/Menardo_2021/menardo_REAL.tsv", auto_standardize=True)
	merged = Ranchero.merge_dataframes(merged, menardo_2018, merge_upon="sample_id", right_name="Menardo_2018", indicator="collection")
	merged = Ranchero.merge_dataframes(merged, menardo_2021, merge_upon="sample_id", right_name="Menardo_2021", indicator="collection", escalate_warnings=False)
	...
	#merker_samples = Ranchero.from_tsv("./inputs/publications/sample_ided/Merker_2022 [ERS]/Merker_cleaned_ERS_id.tsv", auto_rancheroize=False, glob=False, check_index=False,
	#	drop_columns=["pheno_STREPTOMYCIN","pheno_ISONIAZID","pheno_RIFAMPICIN","pheno_ETHAMBUTOL","pheno_PYRAZINAMIDE",
	#	"pheno_AMIKACIN","pheno_KANAMYCIN","pheno_CAPREOMYCIN","pheno_CYCLOSERINE","pheno_CLOFAZIMINE","pheno_OFLOXACIN",
	#	"pheno_PAS","pheno_RIFABUTIN","pheno_MOXIFLOXACIN","pheno_LINEZOLID","pheno_ETHIONAMIDE","pheno_BEDAQUILINE"]) # turns out these are genotypic
	#merker_ids = Ranchero.from_tsv("./inputs/publications/sample_ided/Merker_2022 [ERS]/ERS-to-SAME-incomplete.txt", auto_rancheroize=False, glob=False, check_index=False)
	...
	ref_set_CSISP = Ranchero.from_tsv("./inputs/publications/sample_ided/ref_set_CSISP.tsv", auto_rancheroize=False, glob=False, check_index=False)
	merged = Ranchero.merge_dataframes(merged, ref_set_CSISP, merge_upon="sample_id", right_name="ref_set_CSISP", indicator="collection")
	check_stuff(merged)
	...
	start, merged = time.time(), Ranchero.merge_dataframes(merged, standford_1, merge_upon="sample_id", right_name="standford", indicator="collection", fallback_on_left=False, escalate_warnings=False)
	print(f"Merged with standford1 in {time.time() - start:.4f} seconds")
	start, merged = time.time(), Ranchero.merge_dataframes(merged, standford_2, merge_upon="sample_id", right_name="standford", indicator="collection", fallback_on_left=False, escalate_warnings=False)
	print(f"Merged with standford2 in {time.time() - start:.4f} seconds")
	start, merged = time.time(), Ranchero.merge_dataframes(merged, standford_3, merge_upon="sample_id", right_name="standford", indicator="collection", fallback_on_left=True, escalate_warnings=False)
	print(f"Merged with standford3 in {time.time() - start:.4f} seconds")
	start, merged = time.time(), Ranchero.merge_dataframes(merged, standford_4, merge_upon="sample_id", right_name="standford", indicator="collection", fallback_on_left=True, escalate_warnings=False)
	print(f"Merged with standford4 in {time.time() - start:.4f} seconds")
	start, merged = time.time(), Ranchero.merge_dataframes(merged, standford_5, merge_upon="sample_id", right_name="standford", indicator="collection", fallback_on_left=True, escalate_warnings=False)
	...
	#start = time.time()
	#merged = Ranchero.merge_dataframes(merged, PRJEB9680, merge_upon="sample_id", right_name="PRJEB9680_superset", indicator="collection")
	...
	walker = Ranchero.from_tsv("./inputs/publications/sample_ided/Walker_2022-CRyPTIC-samp/sampleindexed_Walker2022_pheno_WHO2021_CRyPTIC_PMC7612554.tsv", explode_upon=" ")
	...
	start, merged = time.time(), Ranchero.merge_dataframes(merged, walker, merge_upon="sample_id", right_name="Walker_2022", indicator="collection")
	...
	start, merged = time.time(), Ranchero.merge_dataframes(merged, Fran_non_sra, merge_upon="sample_id", right_name="TGU", indicator="collection", drop_exclusive_right=False)
	start, merged = time.time(), Ranchero.merge_dataframes(merged, Fran_sra, merge_upon="sample_id", right_name="TGU", indicator="collection", drop_exclusive_right=False)
	...
	merged = Ranchero.merge_dataframes(merged, inputs, merge_upon="sample_id", right_name="input", indicator="collection", drop_exclusive_right=False)
	...
	tree = Ranchero.from_tsv("./ranchero_output_archive/2025-07-08-FINAL_ranchero_rc17.subset.annotated.tsv", auto_rancheroize=False, list_columns=['run_id'])
	for column in tree.columns:
		if column in merged and column not in ['sample_id']:
			# coinfection and tbprofiler_lineage_usher will get added too
			tree = tree.drop(column)
	merged = Ranchero.merge_dataframes(merged, tree, merge_upon="sample_id", right_name="tree", indicator="collection", drop_exclusive_right=False)
	...
	merged = Ranchero.merge_dataframes(merged, tbprofiler, merge_upon="sample_id", right_name="tbprofiler", indicator="collection", drop_exclusive_right=False)
	
	denylist = Ranchero.from_tsv("./inputs/pipeline/denylist_2024-07-23_lessdupes.tsv", auto_rancheroize=False)
	merged = Ranchero.merge_dataframes(merged, denylist, merge_upon="sample_id", right_name="denylist", indicator="collection", drop_exclusive_right=True)
