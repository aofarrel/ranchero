...
def inital_file_parse():
	...
	print(f"Wrote to disk in {time.time() - start:.4f} seconds")
	print("Checking dataframe in memory:")
	check_stuff(tba6)
	print("Reading disk:")
	what_we_just_wrote = Ranchero.from_tsv("tba6_standardized.tsv", auto_standardize=False)
	print("Checking dataframe on disk:")
	check_stuff(what_we_just_wrote)
	print("Looks good to me dude")
	return tba6
...
def inject_metadata(tba6):
	check_stuff(tba6)
	tba6 = Ranchero.add_column_with_this_value(tba6, "collection", "")
...
if start_from_scratch:
	tba6_standardized = inital_file_parse()
	merged_runs = run_merges(tba6_standardized)
	merged_samps = sample_index_merges(merged_runs)
	tba6_injected = inject_metadata(merged_samps)
	merged_samps = tba6_injected
else:
	exit(1)
...
Ranchero.to_tsv(merged, "./ranchero_rc11.tsv")
