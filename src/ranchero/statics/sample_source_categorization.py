...
# Standardizing sample source is tricky, because what's important to one person
# might be unimportant to another. There's also a lot of scenarios where people
# use ambigious shorthands.
# In attempt to categorize similar isolation sources while keeping specificity,
# ranchero will split off isolation source into up to four(!) columns:
# * sample_source_category
# * sample_source_subcategory
# * sample_source_standardized
# * sample_source_raw
#
...
# PROCESS:
# 1) Collate all typical sample source columns and save to sample_source_raw
# 2) Standardize things that are clearly related, ex, "blood draw" and "blood"
#    should both be considered just "blood" -> sample_source_standardized
# 3) Categorize based on those standardizations -> sample_source_category and
#    sample_source_subcategory

# Below is just the stuff from part 2 since we're mainly thinking about categories right now.
# I already have a pretty comprehensive matching set for part 1.

# So what the user actually sees for each sample is three columns:
# Category: one of [environmental, facility, laboratory, nonspecific, patient, vetrinary, silico] --> in practice for CDPH data it will likely always be either just patient or facility
# Isolation source: output of part #1 (ex: "blood")
# Isolations source raw: exactly what the isolation source was uploaded under, in case part #1 messed up (ex: "blood draw")

# Three is a lot of columns, so we could drop isolation source or isolation source raw if needed

# ENVIRONMENTAL: Soil, river water, anything "outside"
#  * Human-managed ecosystems are considered environmental, such as aquariums
#  * HCU water is considered facility-HVAC
environmental = ['environmental']
environmental_soil = ['soil']
environmental_water = ['aquarium', 'coastline', 'wastewater']

# FACILITY: Hospitals, post-acute, nursing homes, morgues, anything "inside"
#  * Tubes themselves are facility_tubes, the wound they leave on the patient are categorized
#    in patient instead (keep in mind this can be ambigious)
facility = [
	'abbattoir',
	'clinic',
	'emergency room',
...
facility_internal_equipment = ['speculum', 'thermometer']
facility_furniture = [
	'bed',
	'chair',
	'countertop',
...
facility_HVAC = ['heater-cooler unit']
facility_surgical_equipment = ['scapel', 'scissors', 'surgical saw', 'surgical tray', 'tweezers']
facility_tubes = [
	'catheter',
...
# LABORATORY: Experimental evolution, "lab strains"
#  * Excludes cultures taken from patients/facilities etc
laboratory = ['laboratory', 'experimental evolution', 'experimental transformant']

# NONSPECIFIC
# * Anything that is null for isolation source will be tagged as "nonspecific"
nonspecific_culture = ['culture']
nonspecific_biofilm = ['biofilm']

# PATIENT: Anything taken directly from a patient
#  * Equipment that goes inside a patient (catheters, etc) are considered facility
#  * Anything tagged as "clinical" is assumed to be patient
...
patient = [
	...
	'granuloma',
	'patient',
	'scrapate of granuloma',
]
patient_external = exact_replacements_body_parts.values() + [
	'abscess',
	'axilla',
	'nares',
	'groin',
	'lesion',
	'scar tissue',
	'ulcer', # ambigious with internal stomach ucler but we're not dealing with H. pylori
	'wound'
]
patient_feces = ['excreted bodily substance (unspecified)', 'feces']
patient_fluid = [
	'bile',
	'blood (C&S)',
	'blood',
	'bodily fluid (unspecified)',
	'bronchial wash',
	'bronchoalveolar aspirate',
...
patient_tissue = ['lymph node', 'muscle']

# VETRINARY: Anything that is very clearly labeled as a vetrinary sample, including
# experiments done on animals
#  * If it is ambigious, it will likely go under patient instead
vetrinary = ['vetrinary']
vetrinary_feces = ['feces (chicken)', 'feces (unspecified animal)']
vetrinary_tissue = 'homogenized mouse spleens'
