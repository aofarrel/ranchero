...
def sample_index_merges(merged_runs):
	...
	merged = merged.filter(pl.col('organism') == pl.lit('Mycobacterium canettii'))
	print(f"Of which {merged.shape[0]} samples are canettii")
	return merged
...
merged.select('sample_index')
Ranchero.to_tsv(merged, f"./ranchero_{rc}_just_cannettii_diffs.tsv")
