...
print(f"--> We have pDST data for {rc17_tree_only.filter(pl.col('pDST')).shape[0]} of them")
print(f"--> We have country metadata for {rc17_tree_only.filter(pl.col('has_country')).shape[0]} of them")
print(f"--> We have TBProfiler primary lineage data {rc17_tree_only.filter(pl.col('has_tbprof_lin')).shape[0]} of them")
print(f"--> We have host metadata {rc17_tree_only.filter(pl.col('has_host')).shape[0]} of them")
